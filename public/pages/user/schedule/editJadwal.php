...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $schedule->updateSchedule(
            (int) $_POST['id_schedule'],
            $_POST['nama'],
            $_POST['deskripsi'] ?? null,
            $_POST['tanggal'],
            $_POST['jam_mulai'],
            $_POST['jam_selesai'],
            $_POST['remind_before_minutes'] ?? null
        );

        AuthMiddleware::logCRUD('update', 'schedule', $_POST['id_schedule']);
        ...
        <div class="bg-white border-b border-gray-100 px-3 sm:px-4 md:px-6 py-4 sm:py-5 md:py-6">
            <div class="w-full max-w-full mx-auto">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="mt-0">
                        <a href="detailJadwal.php?id=<?= $data['id_schedule'] ?>" 
            ...
            <?php if (!empty($error)): ?>
                <div class="w-full max-w-full mx-auto mb-4 sm:mb-6 p-3 sm:p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <p class="font-medium text-sm sm:text-base">Gagal memperbarui jadwal</p>
            ...
            <div class="w-full max-w-full mx-auto">
                <div class="bg-white border border-gray-200 rounded-xl sm:rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="px-4 sm:px-5 md:px-6 py-4 sm:py-5 border-b border-gray-100 bg-gradient-to-r from-[#2563EB]/5 to-[#93C5FD]/5">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3 sm:gap-4">
                                ...
                                <div>
                                    <h2 class="text-base sm:text-lg md:text-xl font-semibold text-[#1F2937]">Form Edit Jadwal</h2>
                                    <div class="flex items-center flex-wrap gap-2 mt-0.5">
                                <span class="hidden sm:inline text-xs sm:text-sm text-gray-500">Terakhir diubah: <?= date('d/m/Y', strtotime($data['updated_at'] ?? 'now')) ?></span>
                            </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <form method="POST" action="" class="p-4 sm:p-5 md:p-6">
                        <input type="hidden" name="id_schedule" value="<?= $data['id_schedule'] ?>">
                                ...
                                <div class="group">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" 
                                               name="nama" 
                                               required
                                               value="<?= htmlspecialchars($data['nama_schedule']) ?>"
                                               class="pl-10 w-full  py-2.5 sm:py-3.5 bg-white border border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition-all duration-200"
                                ...
                                <div class="group">
                                    ...
                                    <div class="relative">
                                        ...
                                        <textarea name="deskripsi" 
                                                  rows="4"
                                                  class="pl-10 w-full py-2.5 sm:py-3.5 bg-white border border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition-all duration-200 resize-none"
                                                  placeholder="Deskripsi detail tentang kegiatan ini..."><?= htmlspecialchars($data['deskripsi'] ?? '') ?></textarea>
                                    </div>
                                </div>
                                    ...
                                    <div class="group">
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="time" 
                                                   name="jam_mulai" 
                                                   required
                                                   value="<?= date('H:i', strtotime($data['jam_mulai'])) ?>"
                                                   class="pl-10 w-full py-2.5 sm:py-3.5 bg-white border border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition-all duration-200">
                                        </div>
                                    </div>
                                    ...
                                    <div class="group">
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="time" 
                                                   name="jam_selesai" 
                                                   required
                                                   value="<?= date('H:i', strtotime($data['jam_selesai'])) ?>"
                                                   class="pl-10 w-full py-2.5 sm:py-3.5 bg-white border border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition-all duration-200">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="group">
                                    ...
                                    <div class="relative">
                                        ...
                                        <select name="remind_before_minutes"
                                                class="pl-10 w-full py-2.5 sm:py-3.5 bg-white border border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB] transition-all duration-200 appearance-none cursor-pointer">
                                            <option value="">Tidak ada pengingat</option>
                                            <option value="5" <?= ($data['remind_before_minutes'] ?? '') == '5' ? 'selected' : '' ?>>5 menit sebelum</option>
                                            <option value="10" <?= ($data['remind_before_minutes'] ?? '') == '10' ? 'selected' : '' ?>>10 menit sebelum</option>
                                            <option value="15" <?= ($data['remind_before_minutes'] ?? '') == '15' ? 'selected' : '' ?>>15 menit sebelum</option>
                                            <option value="30" <?= ($data['remind_before_minutes'] ?? '') == '30' ? 'selected' : '' ?>>30 menit sebelum</option>
                                            <option value="60" <?= ($data['remind_before_minutes'] ?? '') == '60' ? 'selected' : '' ?>>1 jam sebelum</option>
                                        </select>
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div class="flex items-center gap-3 text-xs sm:text-sm text-gray-600">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span>Mengedit jadwal yang sudah ada</span>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                                    ...
                                    <button type="submit" 
                                            class="order-1 sm:order-2 w-full sm:w-auto px-4 sm:px-8 py-2.5 sm:py-3.5 bg-gradient-to-r from-[#10B981] to-[#34D399] text-white rounded-lg hover:from-[#059669] hover:to-[#10B981] 
